import web3, {consumptionContract} from './contracts.js';
import {currentTime, timeConverter} from './time-format.js';
...
async function consRealTimeEner() {
  consumptionContract.events.ConsTransactionEvent({
    ...
    if (!error) {
      // table starts from here
      header2.push([result.returnValues.Nordic EnergyAddr, timeConverter(result.returnValues.eTime), result.returnValues.enerAmount]);
      //Create a HTML Table element.
